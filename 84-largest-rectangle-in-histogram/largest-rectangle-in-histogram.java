class Solution {
    public int largestRectangleArea(int[] heights) {
        ...
        for (int i = 0; i < heights.length; i++) {
            ...
            // height of the bar at the top of the stack, then we need to compute potential rectangles.
            while (!stack.isEmpty() && stack.peek()[1] > heights[i]) {
                ...
                // so use the entire width up to 'i'. Otherwise, the width is between the current bar and the next shortest bar.
            ...
            // Push the current bar onto the stack. It may be part of a larger rectangle later on.
            stack.push(new int[] { i, heights[i] });
        }

        // Process any remaining bars in the stack. These bars didn't have any shorter bar to their right.
        while (!stack.isEmpty()) {
            // Pop the top bar.
            int[] current = stack.pop();
