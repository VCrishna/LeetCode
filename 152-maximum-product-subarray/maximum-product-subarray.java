class Solution {
    public int maxProduct(int[] nums) {
        ...
        for(int num : nums) {
            // Calculate the temporary product by multiplying the current element with maxProd
            // so the we do not lose maxProd value while calculating minProd as we are going to update it before
            int temp = num * maxProd;

            // updating maxProd to be the maximum of the current element, temp (product of the current element and maxProd),
            // and the product of the current element and minProd
            maxProd = Math.max(num, Math.max(temp, num * minProd));

            // updating minProd to be the minimum of the current element, temp (product of the current element and maxProd),
            // and the product of the current element and minProd
